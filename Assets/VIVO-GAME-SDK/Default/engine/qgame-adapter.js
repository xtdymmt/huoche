...
var image = qg.createImage ? qg.createImage() : new Image();
...
// window.CanvasRenderingContext2D = qg.getCanvasRenderingContext2D() || jsb.
...
var __isCreateMainCanvas = true;
window.mainCanvas = qg.createCanvas ? qg.createCanvas() : creatMainCanvas();

function creatMainCanvas() {
    console.warn(__isCreateMainCanvas);
    if (__isCreateMainCanvas) {
        __isCreateMainCanvas = false;
...
// 联盟统一版本已经挂在在global上，此处做下兼容，防止低版本引擎出现错误
...
// window.addEventListener = function (eventName, listener, options) {
//     window.document.addEventListener(eventName, listener, options);
// };

// window.removeEventListener = function (eventName, listener, options) {
//     window.document.removeEventListener(eventName, listener, options);
// };
...
var Document = function (_Node) {
  ...
  function Document() {
	...
	_this.URL = "http://localhost:8080/index.html"
    _this.scripts = [];
    return _this;
  }
...
if (qg.subscribeNetworkStatus) {
  ...
  try {
    qg.onNetworkStatusChange(function (data) {
      navigator.onLine = data.isConnected;
    });
  } catch (error) {
    console.error(error);
  }
