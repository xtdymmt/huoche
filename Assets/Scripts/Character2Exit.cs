// dnSpy decompiler from Assembly-CSharp.dll class: Character2Exit
using System;
using UnityEngine;

public class Character2Exit : MonoBehaviour
{
	private void Awake()
	{
		...
		this.counter = 0;
		this.CurrentState = Character2Exit.PlayAnim.SitAnim;
	}
	...
	{
		this.CurrentState = Character2Exit.PlayAnim.ExittoStandAnim;
	...
	private void LateUpdate()
	{
		...
		if (this.counter == 1)
		{
			this.CurrentState = Character2Exit.PlayAnim.WalkAnim;
		...
		if (this.counter == 4)
		{
			this.CurrentState = Character2Exit.PlayAnim.IdleAnim;
			this._me.rotation = Quaternion.Slerp(this._me.rotation, Quaternion.LookRotation(this.Target3.position - this._me.position), Time.deltaTime * 9f);
		}
	}
	...
	private Character2Exit.PlayAnim CurrentState;
