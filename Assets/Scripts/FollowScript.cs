// dnSpy decompiler from Assembly-CSharp.dll class: FollowScript
using System;
...
public class FollowScript : MonoBehaviour
{
	...
	private void Sensors()
	{
		...
		if (Physics.Raycast(vector, base.transform.forward, out raycastHit, this.sensorLength))
		{
			if (raycastHit.transform.tag == "Player")
			{
				...
				UnityEngine.Debug.DrawLine(vector, raycastHit.point, Color.blue);
				if (this.HornPlayBool)
				{
					base.GetComponent<AudioSource>().PlayOneShot(this.HornSound, 1f);
					base.Invoke("HornPlayAgain", 10f);
					this.HornPlayBool = false;
				}
			}
		...
		if (Physics.Raycast(vector, base.transform.forward, out raycastHit, this.sensorLength) && (raycastHit.transform.tag == "Player" || raycastHit.transform.tag == "Traffic" || raycastHit.transform.tag == "StopTraffic"))
		{
			this.wheelRL.brakeTorque = this.decellerationSpeed;
			this.wheelRR.brakeTorque = this.decellerationSpeed;
			this.wheelFL.brakeTorque = this.decellerationSpeed;
	...
	private void HornPlayAgain()
	{
		this.HornPlayBool = true;
	}
	...
	public float requiredmaxtorque;

	public AudioClip HornSound;

	private bool HornPlayBool = true;
}
